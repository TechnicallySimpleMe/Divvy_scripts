...
# Initiate Divvy login flow
def get_auth_token():
    resp = requests.post(base_url + '/v3/auth/initiate', json={'username': username})
    token = resp.json().get('token')
    resp = requests.post(base_url + '/v3/auth/provider/ldap', json={'token': token, 'password': password})
...
if not auth_token:
    print("Failed to get an auth token. Exiting")
    exit()
